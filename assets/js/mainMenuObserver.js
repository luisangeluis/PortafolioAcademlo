  ...
  const cbSections = (entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        const currentItem = menuItems.find(item => item.dataset.url === entry.target.id);
        currentItem.firstElementChild.classList.add('active',"color-orange", "zoom-text");
        for (const item of menuItems) {
          if (item != currentItem) {
            item.firstElementChild.classList.remove('active',"color-orange", "zoom-text")
